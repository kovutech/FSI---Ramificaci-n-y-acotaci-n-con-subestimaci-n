...
def ramificacion_acotacion_search(problem):
    return graph_search(problem, AcotQueue())

def ramificacion_acotacion_subestimacion_search(problem):
    return graph_search(problem, AcotSubestQueue(problem))
...
def graph_search(problem, fringe):
    ...
    while fringe:
        node = fringe.pop()
        if problem.goal_test(node.state):
            printf("Number of nodes expanded: %d -->  ", fringe.numberNodes)
            return node
